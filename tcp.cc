...
int Tcp::Listen(int count)
{
    ...
    {
        LOGE("listen error: %s(errno: %d)", strerror(errno), errno);
        return -1;
    }
...
int Tcp::Accept()
{
    ...
    LOGV("accept tcp: %s:%d", remote_ip, ntohs(peeraddr.sin_port));
#endif
    return peerfd;
}
...
int Tcp::InitializeEx(int fd) const
{
    ...
    {
        LOGE("setsockopt TCP_NODELAY error: %s(errno: %d)", strerror(errno), errno);
        close(fd);
        return -1;
    }
    return fd;
